...
const WeatherCard = ({ cityName,lon,lat, country, celsius, fahrenheit, condition }) => {
   ...
   return (
      <div className=" bg-linear-to-tr from-blue-600 to-blue-950 rounded-2xl space-y-3 py-2 px-3 md:space-y-8 w-full md:max-h-5xl h-fit">
         <div className="flex items-center justify-between">
            <div className="space-y-1">
               <h1 className="text-white font-bold text-2xl md:text-4xl">{cityName}</h1>
               <h2 className="text-gray-200 tracking-wider font-light text-xs md:text-sm">{lon} <span>·</span> {lat} {country}</h2>
            </div>
            <div>
         ...
         <div className="space-y-1">
            ...
            <div>
               <p className="text-gray-200 text-sm" >Condition : <span className="text-white font-medium">{condition}</span></p>
            </div>
         </div>
      </div>
   )
